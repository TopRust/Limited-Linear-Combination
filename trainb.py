...
# task testb trainb.py
# task testb1 trainb.py
# task testb2 trainb.py
# task testb3 trainb.py
# task testb4 trainb.py
# task testb5 trainb.py
# task testb6 trainb.py
...
def swap_criterion(criterion, pred, y_a, y_b, b):
    return (1 - b) * criterion(pred, y_a) + b * criterion(pred, y_b) 
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        targets_a, targets_b, b = swap_data(inputs, targets)
        inputs, targets_a, targets_b = map(Variable, (inputs,
                                                      targets_a, targets_b))
        outputs = net(inputs)
        loss = swap_criterion(criterion, outputs, targets_a, targets_b, b)
        ...
        correct += (predicted.eq(targets_a.data).cpu().sum().float()
                    - b * predicted.eq(targets_b.data).cpu().sum().float())
